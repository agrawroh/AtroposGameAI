   ...
   */
    public AtroposCircle getNextPlay(AtroposState state) {
        ...
        try {
            ...
            BufferedInputStream output = new BufferedInputStream(
                    turn.getInputStream());

            //wait for the program to finish
            turn.waitFor();
            BufferedInputStream oerr = new BufferedInputStream(
                    turn.getErrorStream());
            //System.err.println(output_err.toString());
            //read the result of the program
            int nextInt;
            char nextChar;
            for (nextInt = output.read(); nextInt != -1; nextInt = output
                    .read()) {
        ...
        AtroposCircle circle = new AtroposCircle(color, height, leftDistance,
                rightDistance);
        return circle;
    }
