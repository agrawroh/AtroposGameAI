package com.agrawroh.game.atropos.state;
...
public class AtroposState {
    ...
    public AtroposState(int size) {
        ...
        for (int i = 1; i < size; i += 2) {
            this.circles[0][i] = new AtroposCircle(RED, 0, i, size - i);
        }
        for (int i = 2; i < size; i += 2) {
            this.circles[0][i] = new AtroposCircle(BLUE, 0, i, size - i);
        }
        //color the left side green and red
        for (int i = 1; i < size; i += 2) {
            this.circles[i][0] = new AtroposCircle(GREEN, i, 0, size - i);
        }
        for (int i = 2; i < size; i += 2) {
            this.circles[i][0] = new AtroposCircle(RED, i, 0, size - i);
        }
        //color the right side blue and green
        for (int i = 1; i < size; i += 2) {
            this.circles[i][size - i] = new AtroposCircle(BLUE, i, size - i, 0);
        }
        for (int i = 2; i < size; i += 2) {
            this.circles[i][size - i] = new AtroposCircle(GREEN, i, size - i, 0);
        }
        //create uncolored circles for the rest of the board
        for (int height = 1; height < size - 1; height++) {
            for (int leftDistance = 1; leftDistance < size - height; leftDistance++) {
                this.circles[height][leftDistance] = new AtroposCircle(
                        UNCOLORED, height, leftDistance, size - height
                                - leftDistance);
            }
        }
        this.lastPlay = null;
     ...
     * Options for the next play
     */
    public List<AtroposCircle> getPlayableCircles() {
        List<AtroposCircle> list = new ArrayList<>();
        for (int height = 1; height < this.circles.length; height++) {
            for (int leftDistance = 1; leftDistance < this.circles.length
                    - height; leftDistance++) {
                if (this.isLegalPlayLocation(height, leftDistance)) {
                    list.add(this.circleAt(height, leftDistance));
                }
            }
        }
        return list;
     ...
     *            Distance of the circle from the left.
     */
    public boolean circleIsColored(int height, int leftDistance) {
    ...
    public String asString() {
        ...
        result.append("]");
        result.append("LastPlay:(").append(lastPlay.getColor()).append(",")
                .append(lastPlay.height()).append(",")
                .append(lastPlay.leftDistance()).append(",")
                .append(lastPlay.rightDistance()).append(")");
